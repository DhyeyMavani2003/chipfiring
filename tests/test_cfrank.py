...
def test_rank_1_k2_graph_riemann_roch_theorem(k2_graph):
    ...
    assert rank(K_minus_D).rank == rank(D).rank - 1 - D.get_total_degree() + k2_graph.get_genus()
...
def test_rank_k3_slightly_more_chips_riemann_roch_theorem(simple_graph):
    ...
    # Check if Riemann-Roch theorem holds
    assert rank(K_minus_D).rank == rank(D).rank - 1 - D.get_total_degree() + simple_graph.get_genus()
...
def test_rank_sequence_test_graph_riemann_roch_theorem(sequence_test_graph, sequence_test_initial_divisor):
    ...
    # Check if Riemann-Roch theorem holds
    assert rank_K_minus_D.rank == rank_D.rank - 1 - D.get_total_degree() + sequence_test_graph.get_genus()
...
def test_rank_sequence_optimized_corollary_4_4_3(sequence_test_graph):
    ...
    D = CFDivisor(sequence_test_graph, [("Alice", 5), ("Bob", -3), ("Charlie", 4), ("Elise", -1)])
    assert rank(D, optimized=True).rank == D.get_total_degree() - sequence_test_graph.get_genus()
