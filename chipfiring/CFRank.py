...
class CFRank:
    ...
    def _calculate_rank(self, divisor: CFDivisor, optimized: bool = False) -> 'CFRank':
        ...
        if optimized:
            self.log("Optimized mode is enabled. Checking if we can apply theoretical shortcuts before proceeding.")
            
            D = self._divisor
            # Using Corollary 4.4.3 from Dhyey Mavani's Math thesis:
            if D.get_total_degree() > 2 * graph.get_genus() - 2:
                self.log("Optimized mode: D has degree > 2g-2. Using Corollary 4.4.3 from Dhyey Mavani's Math thesis to skip step 2, and return rank(D) = degree(D) - genus(G).")
            ...
            if (K - D).get_total_degree() < D.get_total_degree():
                self.log("Optimized mode: (K-D) has lower degree than D. Running next step on (K-D).")
                self._divisor = K - D
            else:
                self.log("Optimized mode: (K-D) has degree >= that of D. Running next step on D itself.")
            ...
            try:
                ...
                with Pool() as pool:
                    ...
                    for winnable_result in results_iterator:
                        ...
                        self.log(f"    Processed (k={k}, item {num_divisors_processed_for_k}): Winnable -> {winnable_result}")
                        if not winnable_result:
                            any_unwinnable_found_for_k = True
                            self.log(f"    Found unwinnable divisor for k={k}. Terminating pool for this k.")
                ...
                self.log(f"  Multiprocessing failed for k={k}: {e}. Falling back to sequential execution.")
                ...
                for sub_divisor in generate_valid_test_divisors_for_current_k():
                    ...
                    self.log(f"    Processed (k={k}, item {num_divisors_processed_for_k}): Divisor {sub_divisor.degrees_to_str()} -> Winnable: {winnable_res}")
                    if not winnable_res:
                        any_unwinnable_found_for_k = True
                        self.log(f"    Found unwinnable divisor {sub_divisor.degrees_to_str()} for k={k}.")
                        break
                self.log(f"  Sequential processing finished for k={k}.")
            
            if not processed_at_least_one_valid_divisor:
                self.log(f"  For k={k}, no valid test divisors were generated (e.g., k too large). Rank: {k-1}")
                self._rank_value = k - 1
                return self
            
            if any_unwinnable_found_for_k:
                self.log(f"  For k={k}, an unwinnable configuration was found. Rank: {k-1}")
                self._rank_value = k - 1
                return self
            else:
                self.log(f"  All {num_divisors_processed_for_k} processed configurations for k={k} were winnable. Incrementing k.")
                k += 1
